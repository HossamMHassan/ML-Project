...
The matrices of confusion show that the algorithm of Random Forest performs better than the trees of decision. The Random Forest model accuracy was 0.995 (95 percent CI: (0.993, 0.997)) versus 0.739 (95 percent CI: (0.728, 0.753)) for Decision Tree model. The model of the random Forest is chosen.

### Expected out-of-sample error
The predicted error in the out-of-sample is measured at 0.005, or 0.5%. For predictions made against the cross-validation set, the expected out-of-sample error is calculated as 1-accuracy. There are 20 cases in our Test Data Collection. With more than 99 per cent accuracy on our cross-validation results, we can expect very few, or none, of the test samples to be misclassified.

## Submission
In this section the values estimated on the test data using the random forest algorithm for the project submission.
